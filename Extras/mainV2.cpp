#include <iostream>
#include <string>
#include <fstream>
//#include <bits/stdc++.h>
using namespace std;

template<typename X>
class GenStack{

  private:
    X *arr;
    int top; //the current location of the top of the stack
    int capacity; //the physical amount of memory allocated for the stack
    int size; // the current size of the stack (top + 1)
    int increase = 4;

  public:
    //default constructor
    GenStack();

    //constructor
    GenStack(int passSize){
      arr = new X[passSize];
      capacity = passSize;
      top = -1;
      size = 0;
    };

    //adds an element to the stack
    void push(X newAdd){
      size++;
      if (isFull()){
        arr = create_new(arr);
      }
      arr[++top] = newAdd;
      //cout << "The element < " << newAdd << " > has been added to the stack" << endl;
    };

    //increases the size of the stack by 4
    X* create_new(X* a){
      //create the new stack
      X* new_arr = new X[getSize() + increase];
      //copying contents of old stack
      for (int i = 0; i < getSize(); i++)
        new_arr[i] = arr[i];
      capacity += increase;
      return new_arr;
    };

    //removes the top element of the stack by moving the top marker down 1
    void pop(){
      if (top - 1 == -2){
        cout << "\n";
        throw out_of_range("The stack is already empty");
      }
      //cout << "The Element < " << peek() << " > has been removed from the stack" << endl;
      top--;
      size--;
    };

    //returns the top element of the stack
    X peek(){
      if (!isEmpty())
        return arr[top];
      else
        cout << "Nothing returned (The Stack is Empty)" << endl;
        return NULL;
    };

    //returns how big the stack has gotten
    int getSize(){
      return size;
    };

    int getTop(){
      return top;
    }

    int getCapacity(){
      return capacity;
    }

    //checks to see if the stack is empty
    bool isEmpty(){
      return getSize() == 0;
    };

    //checks to see if the stack is full
    bool isFull(){
      return getSize() == capacity;
    };

    void printContents(){
      for (int i = 0; i < size; i++) {
        cout << arr[i] << endl;
      }
    }

    //destructor
    ~GenStack(){
      delete[] arr;
    }
};

class delimeterPair{

  private:
    int alphaLine;
    int alphaLocation;
    int betaLine;
    int betaLocation;

  public:
    delimeterPair();

    delimeterPair(int passAlphaLine, int passAlphaLocation){
      alphaLine = passAlphaLine;
      alphaLocation = passAlphaLocation;
    };

    void foundPair(int passBetaLine, int passBetaLocation){
      betaLine = passBetaLine;
      betaLocation = passBetaLocation;
    }
};

void printCode(string fileName){
  cout << "Here is the source code: " << endl;
  cout << "\n";
  ifstream sourceCodePrint(fileName);
  string codeLine;

  while(getline(sourceCodePrint, codeLine)){
    cout << codeLine << endl;
  }
  cout << "\n";
  sourceCodePrint.close();
}

int main(int argc, char *argv[] ){
  cout << "\n===============================================================\n";
  cout << "          Welcome to Michael's Magical Delimeter Check!";
  cout << "\n===============================================================\n";
  cout << "\n";

  cout << "Please enter the name of your source code file: ";
  string fileName = "";
  getline(cin, fileName);
  cout << "\n";

  ifstream sourceCode(fileName);
  string codeLine;

  //printCode(fileName);

  GenStack<string> myStack(5);

  int currentLineNumber = 1;
  while(getline(sourceCode, codeLine)){
    int lineLength = codeLine.length();
    for (int i = 0; i < lineLength; i++){
      if (codeLine[i] == '{'){
        string leftBrace = "| { | Line: ";
        leftBrace += to_string(currentLineNumber) + " | Postion: " + to_string(i);
        myStack.push(leftBrace);
      } else if (codeLine[i] == '('){
        string leftParentheses = "| ( | Line: ";
        leftParentheses += to_string(currentLineNumber) + " | Postion: " + to_string(i);
        myStack.push(leftParentheses);
      } else if (codeLine[i] == '['){
        string leftBracket = "| [ | Line: ";
        leftBracket += to_string(currentLineNumber) + " | Postion: " + to_string(i);
        myStack.push(leftBracket);
      }
    }
    currentLineNumber++;
  }

  cout << "\n";
  cout << "Here is the Stack: " << "(top = " << myStack.getTop() << ")" << " (size = " << myStack.getSize() << ")" << " (capacity = " << myStack.getCapacity() << ")" << endl;
  myStack.printContents();
  cout << "\n";

  cout <<"Here is the top of the Stack: " << endl;
  cout << myStack.peek() << endl;

  sourceCode.clear(); //resets the input text file
  sourceCode.seekg(0, ios::beg); //resets the input text file

  /* currentLineNumber = 0;
  bool delimeterCheck = true;
  while(getline(sourceCode, codeLine) && delimeterCheck == true){
    int lineLength = codeLine.length();
    for (int i = 0; i < lineLength; i++){
      if (codeLine[i] == '}' && delimeterCheck == true){
        if (myStack.peek() != )
      }
    }
    currentLineNumber++;
  }
} */


  cout << "\n";
  sourceCode.close();
  return 0;
}
